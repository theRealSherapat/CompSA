\chapter{Implementation} % Read: how to achieve the last part when doing the middle part to/with the first part?
...
\besk{about introducing, giving an overview, and describing the developed system \tcol[gray]{(as well as the system sub-constituents — aka. what the system consists of)}. $\implies$ In a nutshell answering: What is the system, and what does it consist of? Why?}
	...
	\begin{figure}[h]
		\centering
			...
			\end{subfigure}
		\caption{Synchronization achieved in a musical robot collective, consisting of M. J. Krzyzaniak and RITMO's musical robots, the Dr. Squiggles.}
		\label{fig:initial}
	\end{figure}
	...
	\label{subsec:agent}
		\besk{about the individual agent with all its attributes and characteristics etc. \tcol[gray]{(as e.g. an oscillator-component (cf. I.A, III.Intro., and 'Implementation' in Nymoen's firefly-paper))}}
		
		Notions like ``agent'', ``robot'', ``firefly'', and ``oscillator'' will be used interchangeably throughout the thesis.
		
		All agents have the ability to listen for such transmitted ``fire''-signals from their neighbours, which they then will use as a trigger to adjust themselves according to some well-designed update-functions to be elaborated upon in Section \ref{sec:phase_methods} and \ref{sec:frequency_methods}.
	...
	What are the signals, how do they look (maybe simulation vs. reality), and how are they sent out? What happens when they are sent out?
	
	These aforementioned audio-signals, also referred to as ``fire''-signals, are transmitted whenever an agent's oscillator \textit{peaks} (i.e. after its cycle or period is finished, having phase $\phi(t)=1$) — or actually every second \textit{peak} due to the system target goal of \textit{harmonic synchrony}, to be elaborated upon in Section \ref{sec:harmonic_synchrony}.
...
\besk{about various methods/algorithms/approaches \tcol[gray]{(the nitty gritty)} implemented to achieve the target/goal state of harmonic synchrony. $\implies$ In a nutshell answer: What different but comparable methods did you use to achieve harmonic synchrony with Phase-Adjustment?}
...
\besk{about various methods/algorithms/approaches \tcol[gray]{(the nitty gritty)} implemented to achieve the target/goal state of harmonic synchrony. $\implies$ In a nutshell answer: What different but comparable methods did you use to achieve harmonic synchrony with Frequency-Adjustment?}
	...
	In order for the musical agents to synchronize their frequencies to each other, they will have to adjust or update their frequencies according to some well-designed update-/adjustment-functions. Three approaches/methods that was implemented to achieve this, or more specificly the goal state of \textit{harmonic synchrony}, is presented now. Notice the increasing degree of \textit{Computational Self-Awareness} endowed in the methods.
	...
	\subsection{Person X's low SA-leveled frequency-updates}
	...
	\subsection{K. Nymoen's middle SA-leveled frequency-updates}
	...
	\subsection{Thorvaldsen's high SA-leveled frequency-updates}
...
As previously mentioned, this state of \textit{harmonic synchrony} is then the system goal state which we want the musical robot-collective to end up in eventually, the sooner the better.
...
Accomodating for these aspects then, K. Nymoen et al. \cite{nymoen_synch} took inspiration for achieving synchronization in a decentralized system from the concept of \textit{harmonics} in the frequency spectrum of a waveform, in that each harmonic wave or overtone has a frequency with an integer-relationship to the fundamental (smallest) frequency. This can e.g. be seen in the frequency spectrogram below, in Figure \ref{fig:G3_waveform_frequency_spectrogram}, where a waveform with a fundamental frequency of 195,99 Hz (i.e. a G3-note and $\approx$ 200 Hz) has harmonics and overtones with frequencies with integer relationships to said fundamental frequency.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\linewidth]{Assets/Figures/G3_196Hz_HummingWaveform_FrequencySpectrum.pdf}
	\caption{The frequency spectrogram of a 14 second long waveform being a hummed and recorded G3-tone at around 195,99 Hz.}
	\label{fig:G3_waveform_frequency_spectrogram}
\end{figure}

This is exactly how the frequencies of the individual musical agents in our music-collective must be, in order for the agent-collective to be harmonically synchronized.

More accurately, all musical agents—in a harmonically synchronized state—will have frequencies $\in \omega_{0} \cdot 2^{\mathbb{N}_0}$, where $\omega_{0}$ is the agent with the lowest frequency's frequency (i.e. the fundamental frequency), and $\mathbb{N}_0$ is the mathematical set of natural numbers including the number zero. Hence, agents will typically have frequencies like $\omega_{0} \cdot 2^0 = \omega_{0}$, $\omega_{0} \cdot 2^1 = 2\omega_{0}$, $\omega_{0} \cdot 2^2 = 4\omega_{0}$, or $\omega_{0} \cdot 2^3 = 8\omega_{0}$. If all agents end up with these kind of frequencies, we say they have ``legal'' and harmonically synchronized frequencies.
...
\besk{``where I present the method used to evaluate the performance of the presented and newly proposed algorithms/methods \tcol[gray]{(i.e. the Synchronization-/Performance-measure)}'' — Samuelsen's MSc-thesis I think}
