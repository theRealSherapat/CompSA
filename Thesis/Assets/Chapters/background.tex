...
\section{Multi-agent systems concepts}
\begin{itemize}
...
\gjor{Forklare hva et signal eller et (lyd-)signal/waveform egentlig er (ift. harmonics/harmonic wave/overtones (og deres typiske integer-relationship frekvenser), timbre med varierende frekvenser (jf. Jay Eller hva han Andriy Burkov eller hva han het anbefalte), amplitude/gain, envelope/attack-decay)}
\inkl{Introen: Inspoen fra kråke-observasjonen fra joggeturen, og andre (inspo-)eksempler på signaler som sendes og typiske responser og deres konsekvenser}

	\subsection{Waveform / Amplitude}
	
	\gjor{Inkludere et Audacity-waveform med forklaringen på plottet (se audacity\_waveform i Audacity-manualen)}
