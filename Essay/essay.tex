...
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{geometry} % for å få en bredere side / mindre marginer
\usepackage[dvipsnames]{xcolor} % for å få til farget tekst
\usepackage[nottoc,numbib]{tocbibind} % for å få References i ToC
\usepackage{titlesec}
...
% SJEKK HVORDAN REFERANSENE SER UT I SACS 2017 BOKA F.EKS.
% MÅ NOK SKIFTE DENNE STYLEN (PÅ GRUNN AV UTSEENDET TIL NAVNENE I REFERANSENE)
...
% KRITIKK:
% - JEG BØR HATT VEKTLAGT MER SKILNADEN MELLOM DET Å *INNEHA/BESITTE DATA*, OG DET Å *HA MULIGHETEN TIL Å KONTINUERLIG INNSAMLE SUBJEKTIV DATA*
% - JEG BØR HATT VEKTLAGT MER DE KONKRETE ANVENDELSENE AV COMPUTATIONAL SELF-AWARENESS, FREMFOR SÅ MYE OG GRUNDIG TEORI (IFØLGE Kyrre)
...
% FRA ESSAY-CHECKLIST: Short description of the problem / challenge for my thesis.

% INTRODUSERE (FORSTÅELIG OG INTRIGUING) SELF-AWARENESS SOM ET SPENNENDE OG RELEVANT KONSEPT. DISKUTERE MOTIVASJONER OG ARGUMENTERBARE FORDELER AV Å ENDOWE ET COMP. SYSTEM MED SELF-AWARENESS (SE reMarkable'N)?
%   - SACS 16 Ch. 2: "Self-awareness concepts from psychology are inspiring new approaches for engineering computing systems which operate in complex dynamic environments."
%   - SACS 17 Ch. 3: "Increased self-awareness in computing systems can be beneficial in several respects, including a greater capacity to adapt, to build potential for future adaptation in unknown environments, and to explain their behaviour to humans and other systems."

% What constitutes the essential ideas behind the solutions to the problem?

% POTENSIELT INTRODUKSJONS-MATERIALE:
%   (i) Often times, scientists have drawn inspiration from various scientific fields -- particularly different fields from ones own -- into their own field, for various reasons. Indeed, the translated concepts (from the one domain to the other) will most likely be accompanied with brand new ways to think about ones own domain, as well as other domains again interacting with it (hence having a real opportunity to start a "domino"-like chain-reaction of new ways to think about things emerging). These new ways to think about things (often in ones own domain) -- apart from being interesting and intriguing -- might be useful, both for ones own field but also for other fields again (especially if thinking long term). For example in the Multi-Agent Systems (MAS) field, it has been a common practice to study complex biological systems in nature, in order to translate these mechanisms into the technology- and engineering-domain (be it the \textit{Ant Colony(?)}, or \textit{Beeclust(?)}). Such bio-inspired algorithms have been -- \textit{and still are(?)} -- some of the most widely used optimization algorithms throughout history.

%   (ii) In this MSc. thesis, we will explore an exciting and relatively new  translation of the concepts and notions regarding \textit{self-awareness} -- as they pertain to humans and animals especially -- from the domain of Psychology, into the domain of Computation and Engineering.

%   (iii) We will in this project attempt to implement and explore whether, and indeed in what way (how), a computational system (like computational agents/agent-collectives in a computer-simulation, and/or even on physical musical robots) can exibit and display Self-Awareness (and corresponding Self-Expressive) capabilities, comparable to how they are perceived in humans and animals.

%   (iv) We will in this thesis project investigate methods and models for computational self-awareness in multi-robot systems, with application to the musical robotics domain.

%   (v) FRA ESSAY OM path planning, EAs og multi-objective optimization (DEFINITIVT IKKE KOPIER, MEN SKRIV OM): This essay attempts to give an overview over the fields of path-planning, evolutionary algorithms and multi-objective optimization, including pointers to recent work in these fields, especially where they intersect. In tradition with other literature relating to evolutionary algorithms, algorithms which are not population-based or otherwise based on principles similar to evolutionary algorithms are called ’classical’ to separate them from evolutionary variants."
...
% TA MED MER BAKGRUNNSSTOFF FRA 'What Is Self-awareness?' I SACS 17 Ch. 3?
...
% \subsection{Related Concepts in other fields}             (?)
% % History and Literature. Kan også sjekke SACS 2017 Ch.1.
...
% SURVEY ALL (already-existing) COMP. SA (ENTEN INDIREKTE SELVOM OPPHAVSMENNENE IKKE BRUKER TERMEN, ELLER DIREKTE) EFFORTS (SOM F.EKS. BLIR GJENNOMGÅTT I SACS 2017 CH. 1 OG 3). KAN NEVNE RELATERT RESEARCH/TEKNOLOGI, OGSÅ DER COMPUTATIONAL SELF-AWARENESS IKKE ER EKSPLISITT NEVNT.
...
% HER LEGGER JEG INN MYE AV FUNDAMENTET OG BAKGRUNNS-TEORIEN JEG STÅR PÅ (IHVERTFALL RAMMEVERK, KONSEPTER OM COMP. SA, OG DESIGN-/BESKRIVELSES-PRINSIPPER).
...
% \textit{DISKUTERE MOTIVASJONER FOR Å ENDOW'E MUSIC SYSTEMS MED COMP. SA.} NEVNE Nymoens FIREFLIES, OG SoloJam.
...
% Nymoen et al. SITT FIREFLY-PAPER \cite{nymoen_synch} (DER JEG KANSKJE OGSÅ NEVNER NOE MED EPISTEMOLOGI \cite{casa} OGSÅ).
...
% SEKSJON 6
\bibliography{Assets/reference_database}
