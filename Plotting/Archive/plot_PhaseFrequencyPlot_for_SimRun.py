...
def plotPhasesAndFrequencies(t, phaseDataMatrix, frequencyDataMatrix, simRun, save_fig_pls):
    ...
    if save_fig_pls == 1:
        plt.savefig("../Synchrony/SavedData/Plots/" + str(frequencyDataMatrix.shape[1]) + "RobotsTerminatedAfter" + str(round(len(t)/samplingRate)) + "s_PhaseFreqPlot.pdf", dpi=300, format="pdf", bbox_inches="tight")
    plt.show()
